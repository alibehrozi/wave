...
#undef LOG_TAG
#endif
#define LOG_TAG "ComplexToMagSquared"

constexpr size_t MAX_MATH_CHUNK = 4096;

ComplexToMagSquared::ComplexToMagSquared(DataType type, const std::string& name)
...
ComplexToMagSquared::~ComplexToMagSquared() {
    stop();
}

void ComplexToMagSquared::reset() {
    std::lock_guard<std::mutex> lock(mutex_);
}

void ComplexToMagSquared::work() {
    if (!is_active()) return;
