...
#undef LOG_TAG
#endif
#define LOG_TAG "MultiplyConst"

constexpr size_t MAX_MATH_CHUNK = 4096;

MultiplyConst::MultiplyConst(DataType type, float constant, const std::string& name)
...
MultiplyConst::MultiplyConst(DataType type, float const_real, float const_imag, const std::string& name)
...
void MultiplyConst::reset() {
    std::lock_guard<std::mutex> lock(mutex_);
}

void MultiplyConst::set_constant(float constant) {
    std::lock_guard<std::mutex> lock(mutex_);
    const_real_ = constant;
    const_imag_ = 0.0f;
}

void MultiplyConst::set_constant(float const_real, float const_imag) {
    std::lock_guard<std::mutex> lock(mutex_);
    const_real_ = const_real;
    const_imag_ = const_imag;
}

void MultiplyConst::work() {
    if (!is_active()) return;
...
template<typename T>
void MultiplyConst::process(T* data, size_t nitems) {
    std::lock_guard<std::mutex> lock(mutex_);
