...
/**
 * Dedicated Finite Impulse Response (FIR) Band-Pass Filter block.
 */
public class BandPassFilter extends FirFilter {
    ...
    /**
     * Create a band-pass FIR filter with custom parameters.
     ...
     */
    public BandPassFilter(@NonNull DataType type,
                          double gain,
                          double samplingFreq,
                          double lowCutoff,
                          double highCutoff,
    ...
    /**
     * Create a band-pass FIR filter with default parameters.
     * @param type            Data type (e.g., FLOAT32, FLOAT64)
     * @param samplingFreq    Audio sample rate in Hz (e.g., 44100)
     * @param lowCutoff       Low cutoff frequency in Hz
     * @param highCutoff      High cutoff frequency in Hz
    ...
    /**
     * Update filter passband frequencies dynamically at runtime.
    ...
    public void setFrequencies(double lowCutoff, double highCutoff) {
        this.lowCutoff = lowCutoff;
        this.highCutoff = highCutoff;
        setTaps(FilterDesign.bandPass(this.gain,
                this.samplingFreq,
                this.lowCutoff,
                this.highCutoff,
