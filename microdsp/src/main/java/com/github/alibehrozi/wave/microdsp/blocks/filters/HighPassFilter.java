...
/**
 * Dedicated Finite Impulse Response (FIR) High-Pass Filter block.
 */
public class HighPassFilter extends FirFilter {
    ...
    /**
     * Create a high-pass FIR filter with custom parameters.
     ...
     * @param interpolation   Interpolation factor (rate increase, 1 = no
     *                        interpolation)
     * @param name            Block name
     */
    public HighPassFilter(@NonNull DataType type, double gain, double samplingFreq, double cutoffFreq,
                          double transitionWidth, FilterDesign.WindowType windowType,
                          int decimation, int interpolation, String name) {
        super(type, FilterDesign.highPass(gain, samplingFreq, cutoffFreq, transitionWidth, windowType),
                decimation, interpolation, name);
        this.gain = gain;
    ...
    /**
     * Create a high-pass FIR filter with default parameters.
     * @param type            Data type (e.g., FLOAT32, FLOAT64)
     * @param samplingFreq    Audio sample rate in Hz (e.g., 44100)
     * @param cutoffFreq      Cutoff frequency in Hz
     * @param transitionWidth Relative transition width (0.0 to 1.0, default 0.05)
     */
    public HighPassFilter(@NonNull DataType type, double samplingFreq, double cutoffFreq, double transitionWidth) {
        this(type, 1.0, samplingFreq, cutoffFreq, transitionWidth, FilterDesign.WindowType.HAMMING, 1, 1,
                "high_pass_filter");
    }
     ...
     * @param cutoffFreq   Cutoff frequency in Hz
     */
    public HighPassFilter(@NonNull DataType type, double samplingFreq, double cutoffFreq) {
    ...
    public void setCutoffFrequency(double newCutoffFreq) {
        ...
        setTaps(FilterDesign.highPass(this.gain, this.samplingFreq, this.cutoffFreq, this.transitionWidth,
                this.windowType));
    }
